from AuxiliaryFunctions import get_all_renamed_words, is_word_regular, get_reduced_set_of_good_word
...
w_length = 13
checklist3 = get_reduced_set_of_good_word(w_length)
sol_dict = dict()
for w in checklist3:
    l = [w,]
    ...
    with gp.Env(empty = True) as env:
        ...
        with gp.Model(env = env) as model:
            vars = model.addMVar(shape = (1, l), vtype = GRB.INTEGER, lb = 0)
...
with open(f'reduced_list_of_length_{w_length}_words.txt', 'w') as sourceFile:
    for i in sol_dict.keys():
        sourceFile.write(f'Word: {i}, List of cycles: {sol_dict[i]}\n')
    sourceFile.close()
